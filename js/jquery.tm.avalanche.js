...
;(function( $, document, window, undefined ) {
	...
	var AvalancheSlider = function( element, options ){
		...
		// Determine first slide and if a particular slide is being linked to
		var url = window.location.hash, id = url.indexOf( '#' );
		var hash = id != -1 ? url.substring( id + 1 ) : '';
		var active;
		if( hash !== '' && slider.find( '#' + hash ).length ){
			history.pushState( '', document.title, window.location.pathname + window.location.search );
			active = slider.find( '#' + hash ).addClass( 'active' );
		}else{
			active = slides.children( 'li:first-child' ).addClass( 'active' );
		}
		...
		if( settings.navPagination && slides.children().length > 1 ){
			...
			pagination.children().eq( slider.find( 'li.active').index() ).addClass( 'active' );
			...
			if( slide.find( 'iframe' ).length ){	
				...
				if( typeof url != 'undefined' && url.indexOf( 'vimeo' ) > -1 ){
					...
					if( status === 'play' && settings.autoPlay ){
						vPlayer[ player ].play().catch( function( error ) {
							switch ( error.name ) {
								case 'PasswordError':
	...
	*/
	...
	var addVideoEvents = function ( slider, video, settings, obj ){
		...
		if( typeof url != 'undefined' && url.indexOf( 'vimeo' ) > -1 ){
			...
			video.attr( 'src', video.attr( 'src' ).replace( '?api=1','' ) + '?&amp;player_id=' + video.attr( 'id' ) ).addClass( 'vimeo' );
			...
			if ( !vimeo ){
				...
				v.src = protocol + '//player.vimeo.com/api/player.js';
			...
			$( '.vimeo' ).each( function(){
				$( this ).on( 'load', function(){
					...
					var firstSlide = slide.hasClass( 'active' ) ? true : false;
					vPlayer[ id ] = new Vimeo.Player( $( id ) );

					// Add listeners
					// Play if first
					if ( firstSlide && settings.autoPlay ){ 
						vPlayer[ id ].play().catch( function( error ) {
							switch ( error.name ) {
								case 'PasswordError':
									console.log( 'The video is password-protected and the viewer needs to enter the password first' );
									break;
					...
					vPlayer[ id ].setVolume(1);
					if( settings.muteBkgVideo || slide.is( '[data-mute-video]' ) ) vPlayer[ id ].setVolume(0);

					// Playing
					vPlayer[ id ].on( 'progress' , function( event ){
