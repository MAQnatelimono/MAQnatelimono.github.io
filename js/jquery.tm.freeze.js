...
;(function( $, document, window, undefined ) {
	...
	var Freeze = function( element, options ){
		...
		// Override defaults
		settings.useCSS = element.is( '[data-use-css]' ) ? true : settings.useCSS;
		settings.extraSpaceTop = element.data( 'extra-space-top' ) ? element.data( 'extra-space-top' ) : element.css( 'top' ) != 'auto' ? parseInt( element.css( 'top' ), 10 ) : settings.extraSpaceTop;
		...
		if( cssStickySupport && settings.useCSS ){
			element.css({ top: settings.extraSpaceTop + 'px' });
		}else{
