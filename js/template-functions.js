...
$( document ).ready( function(){
	...
	var excludeLink = '.no-page-fade, .no-page-fade a, .side-nav-show, .overlay-nav-show, .mail-link, .lightbox-link, .contains-sub-menu, .blog .pagination-previous, .blog .pagination-next, .disabled, .scroll-link, .navigation-show a, a.navigation-show, .social-list a';
	...
	var templateFunctions = {
		init: function(){
			...
			templateFunctions.countdown( '.countdown' );
			// Horizon
		...
		fullScreenSection: function(){
			...
			var scaleBkgImage = function(){
				$( fsSection ).each( function(){
					...
					var bkgImgWrapper = bkgFsWrapper.find( '.background-image-wrapper' );
					...
					if( $( this ).is( '[data-width],[data-height]' ) ){
						...
						if( $( 'body' ).hasClass( 'mobile' ) ||  winW <= 768 ){
							bkgImgWrapper.css({ height: bkgFsWrapper.height() + 'px' });
							bkgImg.css({ height: 'inherit' });
						}else{
							bkgFsWrapper.removeClass( 'fs-image-scale' );
							bkgFsWrapper.css({ height: '' });
							bkgImg.css({ height: '', minHeight: '' });
		...
		scrollToSection: function(){
			...
			var updateElementState = function( element ){
				...
				// the main menu
				var navigation = 'header, .header-sub, .side-navigation-wrapper, .overlay-navigation-wrapper, .scroll-navigation';
			...
			$( mapContainer ).each( function(){
				...
				for ( var i = 0; i < mapLocations.length; i++ ) {
					...
					google.maps.event.addListener( locationMarker, 'click', ( function( locationMarker, i ) {
						return function() {
							infowindow.setContent( map, mapInfoContent[ i ] );
